...
WITH p.name AS Nev , COUNT(m.title) AS Db //Ez egy segédlekérdezés
WHERE Db > 1 //Így már havinget jelent itt a where
RETURN Nev, Db
...
```js
PROFILE //Végrehajtási terv megjelenítése
MATCH (p:Person)
