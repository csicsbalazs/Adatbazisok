...
SELECT type, release_year, COUNT(*)
          ... FROM movies_and_tv
          ... GROUP BY type, release_year; //GROUP BY-olni csakis a partition key-ekkel lehet Cassandrában és HAVING opció nincs
```
...
SELECT nev
          ... FROM szemely
          ... WHERE foglalkozas IN ('pincer', 'Lakatos'); //Cassandrában nincsen OR operátor, ha mégis lefut az fura, helyette ezt kell használni
```
...
```js
ALTER TABLE szemely
          ... ADD vegzettseg LIST<TEXT>;  //Létrehozunk egy új listát ami szöveges adatok listája lesz
UPDATE Szemely
          ... set vegzettseg = vegzettseg + ['gepesz', 'muszeresz'] //Hozzá fűzzük az új értékeket
...
update szemely
          ...
          ... WHERE nev = 'Nagy Ivo' AND szulev = 1999 AND foglalkozas = 'diak'; //Where feltételnél itt mindent oda kell írni ami primary key-ben van
```
